...
pub(crate) fn print_findings(findings: &[ara_types::Finding], risk_level: RiskLevel) {
    let reset = "\x1b[0m";
    for f in findings {
...
mod tests {
    ...
    fn test_print_findings_does_not_crash() {
        let findings = vec![ara_types::Finding {
            pattern: "eval-usage".into(),
            severity: ara_types::RiskLevel::Critical,
            location: Some("index.js:1".into()),
            description: "eval detected".into(),
        }];
        print_findings(&findings, ara_types::RiskLevel::Critical);
    }
}
